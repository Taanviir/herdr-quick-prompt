...
// A worker deletes its request on success and renames it on failure, so a plain
// request file left behind belongs to one that was killed before it could do
// either. The longest a live launch can take is a couple of minutes, so an hour
// is well clear of anything still in flight.
...
function sweepStaleRequests(maxAgeMs = STALE_REQUEST_MS) {
  ...
  try {
    for (const name of fs.readdirSync(STATE_DIR)) {
      ...
      try {
        if (fs.statSync(file).mtimeMs < cutoff) discardRequest(file);
...
// Only the newest draft is ever offered, so anything older would sit in the
// state directory forever. Sweep those, and any that no longer parse, on the
// way past.
  ...
  try {
    ...
    for (const name of files) {
      ...
      try {
        ...
        if (typeof request.prompt === "string" && typeof request.kind === "string") {
          found = { file, request };
          continue;
        }
...
module.exports = { STATE_DIR, readPrefs, remember, writeRequest, readFailedRequest, discardRequest, finishRequest, sweepStaleRequests };
